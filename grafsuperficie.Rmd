...
oleo<-oleo%>%
  mutate(
    ...
    estilo=as.numeric(estilo))
```

Veremos como o R interpreta as variáveis agora:

```{r}
str(oleo)
```

Vamos rodar os dois modelos de regressão linear múltipla com e sem a _dummie_ de estilo das casas:^[Lembre-se que _partindo de uma amostra de 15 casas, nas quais as casas de números 1, 4, 6, 7, 8, 10 e 12 são em estilo colonial_(...)]
...
```{r message=FALSE}
library(plotly)
plot_ly(data = oleo, z = ~ qtdisolamento, x = ~ temperatura, y = ~ consumooleo, color=~estilo, colors = c('#0C4B8E' ,'#BF382A'),opacity = 0.5) %>%
...
library(plotly)
# draw 3D scatterplot
p <- plot_ly(data = oleo, z = ~ qtdisolamento, x = ~ temperatura, y = ~ consumooleo, opacity = 0.6) %>%
  add_markers() 
p
```
...
# Passo a passo no R para três variáveis num único gráfico
...
Vamos utilizar o primeiro método de gerar o gráfico. Comecemos com a regressão linear múltipla de nosso modelo:

```{r}
hmod <- lm(consumooleo ~ qtdisolamento + temperatura, data = oleo)
summary(hmod)$coef
```
...
valores_ajustados <- crossing(y_grid, x_grid) %>% 
  mutate(z_grid = beta_chapeu[1] + beta_chapeu[2]*x_grid + beta_chapeu[3]*abs(y_grid))
