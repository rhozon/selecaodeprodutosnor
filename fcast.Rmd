...
dygraph(ret(Cl(CORN)), group = "grupo") %>% dyRangeSelector() # Grafico dos retornos do Fechamento

dygraph(Vo(CORN), group = "grupo") %>% dyRangeSelector()  # Grafico do Volume negociado
dygraph(ret(Vo(CORN)), group = "grupo") %>% dyRangeSelector() # Grefico dos retornos do Volume de negociacao
...
ggplotly(
ggAcf(ret(CORN[,"CORN.Close"]), na.action = na.interp) # Funcao de Autocorrelacao dos retornos do Fechamento
)

ggplotly(
ggAcf(ret(CORN[,"CORN.Volume"]), na.action = na.interp) # Funcao de Autocorrelacao dos retornos do Volume
)
...
ret(CORN[,"CORN.Close"]) %>% ur.kpss() %>% summary() # Teste para retorno dos precos

ret(CORN[,"CORN.Volume"]) %>% ur.kpss() %>% summary() # Teste para retorno do volume
...
ret_fech_diario <- ts(ret(Cl(CORN)))
ret_vol_diario <- ts(ret(Vo(CORN)))
