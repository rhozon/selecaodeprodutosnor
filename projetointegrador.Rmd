...
as_tibble(multiplica.elementos) # Mostra o resultado como um vetor
```
...
Esse resultado da média aponta para esse grupo de indivíduos dessa amostra como dentro dos padrões médios do IMC;

Caso você deseje saber se está dentro ou fora dos padrões, segue a tabela de classificação do IMC:

| Resultado           | Situação                  |
| ------------------- | ------------------------- |
|  |
| Abaixo de 17        | Muito abaixo do _peso_    |
| Entre 17 e 18,49    | Abaixo do _peso_          |
| Entre 18,50 e 24,99 | _Peso_ normal             |
| Entre 25 e 29,99    | Acima do _peso_           |
| Entre 30 e 34,99    | _Obesidade_ I             |
| Entre 35 e 39,99    | _Obesidade_ II (severa)   |
| Acima de 40         | _Obesidade_ III (mórbida) |

<small>**Fonte:** [Wikipedia](https://pt.wikipedia.org/wiki/%C3%8Dndice_de_massa_corporal) </small>

Por mais que não se considere importante, sabermos se existe algum indivíduo dentro ou fora dos padrões, podemos criar uma variável(coluna) que mostra isso na tabela:
...
tabela<-tabela%>%
  mutate(Situacao = case_when(
                              ...
                              IMC >= 35 & IMC <= 39.99 ~ "Obesidade II (severa)",
                              IMC > 40 ~ "Obesidade III (mórbida)"))
```

Vamos dar mais uma olhada no dataset:

```{r comment=FALSE, comment=NA, null_prefix=TRUE}
# Suprimo algumas colunas pra printar a tabela com o comando do R

tabela<-tabela%>%
  mutate(IMC=round(IMC,1)) # Arredondo o IMC pra 1 casa apos a virgula

tabelaselecionada<-tabela%>%
  select(Sexo,Idade,Altura,Peso,IMC,Situacao)

tabelaselecionada
```

Veja como a média dos IMCs se comparado com a tabela de situação e classificação se confirma. Não temos nenhum dos indivíduos dessa amostra como _outlier_ ou fora do padrão de peso considerado normal pela classificação.

<center>
![](https://rodolfoscatolon.com.br/wp-content/uploads/2015/03/imc-2.jpg)
</center>


Todavia, essa análise precisa ser avaliada com cautela, uma vez que o IMC não distingue com precisão indivíduos com mais massa muscular e baixo percentual de gordura, peso ósseo p. ex. Ou seja, o IMC não distingue massa magra de massa de gordura!
