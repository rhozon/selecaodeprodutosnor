...
h2o.no_progress() # Omite as barras de progresso do h2o
...
+ bivariada $\rightarrow$ CORN.Close _versus_ CORN.Volume no tempo;
...
# Metricas gerais de perfomance

perf_autoML_univariada <- 
  h2o.performance(autoML_univariada, newdata = teste)

perf_autoML_bivariada <- 
  h2o.performance(autoML_bivariada, newdata = teste)
...
# ------- Dados da estimativa do tipo CORN.Close x CORN.Volume + tempo ---------------
...
Utilizaremos o modelo líder da sugestão para construírmos as previsões. Primeiro especificamos o método:
...
Em seguida "treinamos" o modelo especificado:

```{r}

model_fitted_univariado <- model_spec %>%
  fit(CORN.Close ~ index, data = treino)
model_fitted_univariado

predict(model_fitted_univariado, teste)

```

Em seguida faremos sua calibração:
...
CRAN, **h2o package**, in https://cran.r-project.org/web/packages/h2o/index.html 
