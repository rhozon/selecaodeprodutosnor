...
A função ``apriori()`` está embutida no R para minerar conjuntos de itens frequentes e regras de associação usando o algoritmo Apriori. Aqui, ``Groceries`` são os dados da transação. ``parameter`` é uma lista nomeada que especifica o suporte e os mínimos graus de confiança para localizar as regras de associação. O comportamento padrão é minerar as regras com suporte mínimo de 0,1 e 0,8 como a confiança mínima. Aqui, especificamos o suporte mínimo como 0,01 e a confiança mínima como 0,2.

```{r}

regras <- apriori(Groceries, parameter = list(supp = 0.01, conf = 0.2))
